...
def detect(dataset, net, class_agnostic, load_dir, session, epoch, vis, 
    ...
    dataset, _ = dataset_factory.get_dataset("voc_2007_test", add_params, mode='test')
    ...
    for i, data in enumerate(loader):
        ...
        if i%100!=0:
            continue
        print("Detecting on {}th image".format(i))
        total_tic = time.time()
        im2show = cv.imread(dataset.image_path_at(data[4][0]))
        det_tic = time.time()
        with torch.no_grad():
            cls_score, bbox_pred, _, _, _, _, _, debug_info = faster_rcnn(image_data, image_info, None, None, None)
        ...
        if vis:
            result_path = os.path.join(image_dir, 'result', data[4][0] + '_det.jpg')
