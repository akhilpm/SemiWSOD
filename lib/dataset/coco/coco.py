...
class COCO(ImageDataset):
    ...
    def _load_object_proposals(self, image_data):
        if cfg.TRAIN.PROPOSAL_TYPE == 'SS':
            ...
            ss_file_name = 'coco_' + self._image_set + self._year + '.pkl'
            ss_file_path = os.path.join(self._data_path, 'selective_search_data', ss_file_name)
            with open(ss_file_path, 'rb') as fid:
                ss_boxes = pickle.load(fid)
            for i, data in enumerate(image_data):
                ss_boxes_this_image = ss_boxes[data['id']][:2500]
                data['ss_boxes'] = ss_boxes_this_image
        elif cfg.TRAIN.PROPOSAL_TYPE=='CAM_SS':
            print("Loading CAM object proposals")
            ss_file_name = 'cam_proposals_coco'+ self._year
            ss_file_path = os.path.join(self._data_path, 'cam_data', ss_file_name)
            ss_boxes = shelve.open(ss_file_path)
            for i, data in enumerate(image_data):
                data['ss_boxes'] = ss_boxes[data['id']][:, :4]
        else:
    ...
    def _print_detection_eval_metrics(self, coco_eval, log):
        IoU_lo_thresh = 0.5
        IoU_hi_thresh = 0.95
        ...
        log.info(('~~~~ Mean and per-category AP @ IoU=[{:.2f},{:.2f}] '
               '~~~~').format(IoU_lo_thresh, IoU_hi_thresh))
        log.info('Mean: {:.1f}'.format(100 * ap_default))
        for cls_ind, cls in enumerate(self.classes):
            ...
            log.info('class index:{}, class name: {}  {:.1f}'.format(cls_ind, cls, 100 * ap))

        print('~~~~ Summary metrics ~~~~')
        coco_eval.summarize(log)

    def _do_detection_eval(self, res_file, output_dir, log):
        ann_type = 'bbox'
        ...
        self._print_detection_eval_metrics(coco_eval, log)
    ...
    def evaluate_detections(self, all_boxes, output_dir, log):
        ...
        if self._image_set.find('test') == -1:
            self._do_detection_eval(res_file, output_dir, log)
