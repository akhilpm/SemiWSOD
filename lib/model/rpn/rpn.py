...
class _RPN(nn.Module):
    def __init__(self, in_depth):
        ...
        self.RPN_Conv = nn.Conv2d(in_depth, in_depth, kernel_size=3, stride=1, padding=1, bias=True)
        ...
        self.RPN_cls_score = nn.Conv2d(in_depth, self.nc_score_out, kernel_size=1, stride=1, padding=0)

        # define anchor box offset prediction layer
        self.nc_bbox_out = _anchors_per_point * 4 # 4(coords) * 9 (anchors)
        self.RPN_bbox_pred = nn.Conv2d(in_depth, self.nc_bbox_out, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, base_feature, im_info, gt_boxes):
        ...
        rois = self.RPN_proposal((rpn_cls_prob.data, rpn_bbox_pred.data, im_info, cfg_key)) # B * 2000 * 5
        ...
        if self.training:
            ...
            rpn_keep_all = torch.nonzero(rpn_label.view(-1).ne(-1), as_tuple=False).view(-1)
