...
class _AnchorTargetLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        bbox_targets = self._compute_targets_batch(anchors, gt_boxes.view(-1, 4)[argmax_overlaps.view(-1), :].view(
                batch_size, -1, 4))
        #bbox_targets ->  B x A x 4
        ...
        pos_idx = torch.nonzero(labels.view(-1).eq(1), as_tuple=False).view(-1)
