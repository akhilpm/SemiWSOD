...
class _ProposalLayer(nn.Module):
    ...
    def forward(self, input):
        ...
        anchors = generate(feature_h=scores.size(2), feature_w=scores.size(3)) # (H.W.A) * 4
        anchors = anchors.view(1, -1, 4).expand(batch_size, -1, 4).to(bbox_deltas) # B * (H.W.A) * 4
        ...
        scores = scores.permute(0, 2, 3, 1).contiguous() # B * H * W * A
        scores = scores.view(batch_size, -1)  # B * (H.W.A)
        
        # Convert anchors into proposals via bbox transformations
        proposals = bbox_transform_inv(anchors, bbox_deltas) # B * (H.W.A) * 4
